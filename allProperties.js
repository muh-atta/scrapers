...
async function scrapeProperty(url) {
    const browser = await chromium.launch({ headless: true }); // Keep headless: false for visual debugging
    const page = await browser.newPage();

    try {
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 60000 });

       // --- CORRECTED PROPERTY ID EXTRACTION ---
        let propertyId = null;
        const lastUrlSegment = url.split('/').pop(); // Gets 'تفاصيل-87607079.html' or 'details-87607079.html'

        if (lastUrlSegment) {
            // This regex captures digits occurring before '.html' at the end of the segment
            const match = lastUrlSegment.match(/(\d+)\.html$/);
            if (match && match[1]) {
                propertyId = match[1]; // match[1] contains the captured digits (the ID)
            }
        }

        if (!propertyId) {
            console.error(`Could not extract property ID from URL: ${url}. Skipping this property.`);
            return; // Skip this property if ID cannot be extracted
        }
...
async function collectPropertyUrls(listingPageUrl) {
    const browser = await chromium.launch({ headless: true }); // Can be headless for just collecting URLs
    const page = await browser.newPage();

    let propertyUrls = [];

    try {
        console.log(`Navigating to listing page: ${listingPageUrl}`);
        await page.goto(listingPageUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });

        // --- Strategy 1: Extract from JSON-LD script (Most reliable) ---
        // This is a common and robust way to get structured data if available.
        console.log('Attempting to extract URLs from JSON-LD script...');
        const jsonLdScriptContent = await page.evaluate(() => {
            const scriptTag = document.querySelector('script[type="application/ld+json"]');
            if (scriptTag) {
                try {
                    const json = JSON.parse(scriptTag.textContent);
                    // Check if it's an ItemList and has itemListElement
                    if (json && Array.isArray(json.itemListElement)) {
                        return json.itemListElement.map(item => item.url).filter(url => url); // Ensure URL is not null/empty
                    }
                } catch (e) {
        ...
        if (jsonLdScriptContent && jsonLdScriptContent.length > 0) {
            propertyUrls = jsonLdScriptContent;
            console.log(`Successfully extracted ${propertyUrls.length} URLs from JSON-LD.`);
        } else {
            console.log('No URLs found in main JSON-LD script or script not found. Falling back to direct link extraction.');

            // --- Strategy 2: Extract from <a> tags (Fallback) ---
            // This selects all 'a' tags that have an 'href' attribute starting with '/العقار/تفاصيل-'
            // This pattern implies a property details page in Arabic.
            // For English, it would be '/en/property/details-'
            const links = await page.$$eval('a[href*="/%D8%A7%D9%84%D8%B9%D9%82%D8%A7%D8%B1/%D8%AA%D9%81%D8%A7%D8%B5%D9%8A%D9%84-"]', anchors => {
            ...
            propertyUrls = links;
            console.log(`Extracted ${propertyUrls.length} URLs from <a> tags.`);
        }

        // Clean up URLs: ensure they are absolute and unique
        const uniqueAbsoluteUrls = new Set();
        const baseUrl = new URL(listingPageUrl).origin; // Get the base domain

        propertyUrls.forEach(url => {
            try {
                // Construct full URL if it's relative
                const absoluteUrl = new URL(url, baseUrl).href;
                // Normalize the URL path (e.g., /%D8%A7%D9%84%D8%B9%D9%82%D8%A7%D8%B1/ becomes /en/property/ for consistency if needed)
                // For Bayut.sa, they seem to redirect the Arabic path to the English one if you navigate directly.
                // So, just taking the absolute URL should be fine.
                uniqueAbsoluteUrls.add(absoluteUrl);
            } catch (e) {
                console.warn(`Invalid URL found: ${url}`);
            }
        });

        propertyUrls = Array.from(uniqueAbsoluteUrls);
        console.log(`Final unique property URLs collected: ${propertyUrls.length}`);

    } catch (error) {
        console.error(`Error collecting URLs from ${listingPageUrl}:`, error);
    } finally {
        await browser.close();
    }
...
(async () => {
    const listingPage = '';
    const urls = await collectPropertyUrls(listingPage);
    
    urls.reverse();
    console.log(urls);
    // You can now pass these URLs to your scrapeProperty function one by one.
    // For example:
